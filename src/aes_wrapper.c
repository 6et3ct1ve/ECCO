...
static int parse_key32_from_hex(const char *key_hex, unsigned char key[32]) {
    if (!key_hex || strlen(key_hex) < 64) return 0;
...
char *aes_encrypt_gcm_hex(const unsigned char *plaintext, size_t pt_len,
    ...
    if (!parse_key32_from_hex(key_hex, key)) return NULL;
    unsigned char iv[12];
    ...
    if (!iv_hex || !ct_hex || !tag_hex) {
        free(iv_hex); free(ct_hex); free(tag_hex);
        return NULL;
    }
...
unsigned char *aes_decrypt_gcm_hex(const char *pkg, const char *key_hex, size_t *out_len) {
    ...
    if (!from_hex_n(iv_hex, iv, sizeof(iv)) || !from_hex_n(tag_hex, tag, sizeof(tag))) {
        free(ct_hex);
        return NULL;
    }
